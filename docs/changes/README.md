...
| [CR-SAN-036](CR-SAN-036-provisioning-core.md) | **Provisioning core**: `sandesh init` CLI (migrate→consolidate→reindex→admin; idempotent; `--admin`/`--yes`; CLI-only, never MCP) + **lazy auto-migrate on store open** (apply if `[migrate]`, else install-method-aware actionable error, no self-pip) | provisioning-lifecycle (0.3.0) | COMPLETED | — | 2026-06-15 |
| [CR-SAN-037](CR-SAN-037-install-surface-choice.md) | **install.sh surface choice**: interactive Claude/Pi/both/none prompt (Pi ⇒ no `mcp`; non-interactive override) + delegate provisioning to `sandesh init` + **mandatory migrate when an existing `sandesh.db` is detected** (fail loudly, no silent skip) | provisioning-lifecycle (0.3.0) | COMPLETED | CR-SAN-036 | 2026-06-15 |
| [CR-SAN-038](CR-SAN-038-pi-uvx-and-provision-nudge.md) | **Pi surface**: CLI via `uvx --from 'sandesh-relay[migrate]'` (no `mcp`) when local CLI absent + session-gate **nudge to `sandesh init`** on an unprovisioned store; no init/admin/migrate tool; `[migrate]` error passthrough (bun) + `sandesh init --check` probe (py) | provisioning-lifecycle (0.3.0) | COMPLETED | CR-SAN-036 | 2026-06-15 |
| [CR-SAN-039](CR-SAN-039-docs-restructure.md) | **Docs restructure**: slim README + per-route×surface install guide (`docs/INSTALL.md`/`docs/install/`, install→init→manage→uninstall) + **AUR removed from README** + uninstall matrix | provisioning-lifecycle (0.3.0) | COMPLETED | CR-SAN-036..038, CR-SAN-035 | 2026-06-15 |
| [CR-SAN-040](CR-SAN-040-user-guide.md) | **User guide**: `docs/USER_GUIDE.md` (plain-language, **MCP-users** + **Pi-users** sections — MCP runs `sandesh notify` background loop, Pi native-wakes; `/mcp`-not-`/` note; stop-reason table) + leaner README w/ must-read link + `sandesh notify --help` epilog | patch 0.3.1 | COMPLETED | CR-SAN-041 | — |
| [CR-SAN-041](CR-SAN-041-mcp-agent-surface.md) | **MCP agent surface**: imperative **enable-listening directive** in `instructions` (+ `register`/`setup` tool-desc pointers) so the agent knows to background `sandesh notify`; **5 lifecycle `/` prompts** (`/mcp__sandesh__setup\|register\|unregister\|archive\|unarchive`) — other 7 tools stay tools-only; 12 tools unchanged | patch 0.3.1 | COMPLETED | — | 2026-06-15 |
| [CR-SAN-042](CR-SAN-042-release-set-version.md) | **`release.sh set-version` + finish guard** (Option A): new `set-version <X.Y.Z>` bumps the manual manifests (`integrations/pi/package.json` + `server.json` ×fields, format-preserving, commits) + a `cmd_finish` preflight that refuses a manifest≠tag mismatch (skips absent manifests) — makes the version-sync drift that forced the 0.3.2 hotfix impossible to finish (set-version idempotent; finish guard skips absent manifests) | post-0.3.2 | COMPLETED | — | 2026-06-16 |
| [CR-SAN-043](CR-SAN-043-notifier-lock-resilience.md) | **Notifier lock-contention resilience**: `connect()` sets `PRAGMA busy_timeout=30000` on every connection + `is_locked_error`/`_retry_locked` wrap the five `notifier_*` writes (idempotent, jittered backoff) + `notify.run()` no-flap boundary (retry locked acquire/poll on the cadence, bounded by deadline → exit 2, never exit 1) — fixes watchers dying `database is locked` and flapping `listening` under heavy co-tenant load; AC9 real-contention test (8 threads). No schema change | patch 0.3.3 | COMPLETED | — | 2026-06-24 |
| [CR-SAN-044](CR-SAN-044-mcp-body-field-rename.md) | **MCP `body_text`→`body` field rename** (`sandesh_send` + `sandesh_reply`): straight rename of the two MCP tool params to match the Pi extension, the CLI `--body` flag, and the `sandesh://usage` doc (all already `body`); underlying `sandesh_db.send`/`reply` `body_text` kwarg + CLI unchanged. Breaking MCP param (no alias) — schemas are read at connect-time so real-world breakage is ~nil. No schema change | patch 0.3.4 | COMPLETED | — | 2026-07-05 |
| [CR-SAN-045](CR-SAN-045-project-id-grammar-and-empty-project-tombstone.md) | **Reject un-addressable `project_id` + super-admin archive of zero-address projects** (Option B, unwedge the zombies): `setup()` validates the id against the shared `<Project>` grammar (`_PROJECT`/`PROJECT_RE`, one source with `ADDRESS_RE`); super-admin may **archive** a zero-address project (`_has_any_address`, additive `_archive_guards` carve-out — two-step D4/D6 untouched); CLI `tombstone` idempotent (already-gone → exit 0); PRD D8 amended. No schema change | patch 0.3.5 | COMPLETED | — | 2026-07-20 |
