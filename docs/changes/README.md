...
| [CR-SAN-040](CR-SAN-040-user-guide.md) | **User guide**: `docs/USER_GUIDE.md` (plain-language, **MCP-users** + **Pi-users** sections — MCP runs `sandesh notify` background loop, Pi native-wakes; `/mcp`-not-`/` note; stop-reason table) + leaner README w/ must-read link + `sandesh notify --help` epilog | hotfix 0.3.1 | PENDING | — | — |
