...
| [CR-SAN-033](CR-SAN-033-consolidate-skip-non-stores.md) | **Hotfix 0.2.1**: `consolidate` skips non-store `sandesh.db` files (yoyo-stub/corrupt debris crashed the v0.2.0 installer live) with a CLI notice; real stores unaffected | hotfix 0.2.1 | PENDING | — | — |
