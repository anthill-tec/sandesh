...
| [CR-SAN-030](CR-SAN-030-pre-release-cleanup-sweep.md) | **Cleanup sweep**: redundant in-function imports, `_tombstone_guards`/`search` docstring precision, MCP `reindexed`-flag + `sender`/`since`/`until` coverage, install.sh admin-block heredoc, test_install ResourceWarning | pre-v0.2.0 | IN_PROGRESS | — | — |
