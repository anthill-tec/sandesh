...
| [CR-SAN-026](CR-SAN-026-inbox-filters.md) | **Inbox/fetch filters (lib+CLI)**: composable server-side filters — `sender_project` headline (the cross-project proxy stream) + sender/kind/since/until/subject; filtered fetch marks only the subset; wake path untouched | Wave 7 | IN_PROGRESS | — | — |
