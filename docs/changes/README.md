...
| [CR-SAN-042](CR-SAN-042-release-set-version.md) | **`release.sh set-version` + finish guard** (Option A): new `set-version <X.Y.Z>` bumps the manual manifests (`integrations/pi/package.json` + `server.json` ×fields, format-preserving, commits) + a `cmd_finish` preflight that refuses a manifest≠tag mismatch (skips absent manifests) — makes the version-sync drift that forced the 0.3.2 hotfix impossible to finish | post-0.3.2 | IN_PROGRESS | — | — |
