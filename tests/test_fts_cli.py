...
# ---------------------------------------------------------------------------
# T9 — install.sh content assertions
# ---------------------------------------------------------------------------

class InstallShReindexTest(unittest.TestCase):
    """install.sh must contain a `reindex` invocation that:
    1. Exists in the file.
    2. Is placed AFTER the consolidate block (after the '"$VENV/bin/sandesh" consolidate' line).
    3. Is NOT inside the yoyo-probe conditional
       ('if "$VENV/bin/python" -c "import yoyo' ... 'fi').

    RED: install.sh currently has no reindex invocation at all.
    """
    ...
    def test_reindex_invocation_exists(self):
        """install.sh must contain a 'reindex' invocation.

        RED: no reindex call exists yet in install.sh.
        GREEN must add: `"$VENV/bin/sandesh" reindex` after the consolidate block.
        """
        idx = self._line_index("reindex")
        self.assertGreater(
            idx, -1,
            "install.sh contains no 'reindex' invocation. "
            "GREEN must add: `\"$VENV/bin/sandesh\" reindex` after the consolidate block.",
        )

    def test_reindex_placed_after_consolidate_block(self):
        """The reindex invocation must appear AFTER the consolidate line.

        The consolidate line is: `"$VENV/bin/sandesh" consolidate`
        The reindex line must follow it.

        RED: no reindex line exists → _line_index returns -1.
        """
        consolidate_idx = self._line_index('"$VENV/bin/sandesh" consolidate')
        if consolidate_idx == -1:
            # Fall back to checking for any consolidate line
            consolidate_idx = self._line_index("consolidate")
        reindex_idx = self._line_index("reindex")
        ...
        self.assertGreater(
            reindex_idx, -1,
            "install.sh has no 'reindex' line. GREEN must add it after consolidate.",
        )
        self.assertGreater(
            reindex_idx, consolidate_idx,
            f"reindex (line {reindex_idx + 1}) must appear AFTER "
            f"consolidate (line {consolidate_idx + 1}).",
        )

    def test_reindex_uses_venv_sandesh_binary(self):
        """The reindex invocation must use '$VENV/bin/sandesh' (not bare 'sandesh').

        The form must be: `"$VENV/bin/sandesh" reindex`
        This ensures the correct venv binary runs during installation.

        RED: no reindex line yet.
        """
        # Look for the canonical form
        found = self._line_index('"$VENV/bin/sandesh" reindex')
        if found == -1:
            # Accept the unquoted form too: $VENV/bin/sandesh reindex
            found = self._line_index("$VENV/bin/sandesh\" reindex")
        if found == -1:
            found = self._line_index("VENV/bin/sandesh\" reindex")
        self.assertGreater(
            found, -1,
            "install.sh reindex invocation must use '$VENV/bin/sandesh' binary;\n"
            "expected a line containing '\"$VENV/bin/sandesh\" reindex'.",
        )

    def test_reindex_not_inside_yoyo_probe_conditional(self):
        """The reindex invocation must NOT be inside the yoyo-probe if-block.

        reindex is stdlib-only — it must not require the [migrate] extra.
        The yoyo probe block starts at `import yoyo, jsonschema` and ends at
        the first bare `fi` after it. reindex must appear OUTSIDE that block.

        RED: no reindex line exists yet.
        """
        yoyo_start = self._line_index("import yoyo")
        reindex_idx = self._line_index("reindex")

        self.assertGreater(
            reindex_idx, -1,
            "install.sh has no 'reindex' line. GREEN must add it outside the yoyo block.",
        )

        if yoyo_start == -1:
            # No yoyo probe → reindex placement is fine wherever it is
            return

        # Find the 'fi' closing the yoyo if-block.
        yoyo_fi_idx = -1
        for i in range(yoyo_start + 1, len(self._lines)):
        ...
        if yoyo_fi_idx == -1:
            # Cannot determine block end — skip the placement check.
            return

        inside_yoyo = yoyo_start < reindex_idx < yoyo_fi_idx
        self.assertFalse(
            inside_yoyo,
            f"reindex (line {reindex_idx + 1}) is inside the yoyo-probe conditional "
            f"(lines {yoyo_start + 1}–{yoyo_fi_idx + 1}). "
            "reindex is stdlib-only and must NOT require the [migrate] extra.",
        )

    def test_reindex_not_inside_sandesh_admin_block(self):
        """The reindex invocation must NOT be inside the $SANDESH_ADMIN block.

        The admin block is guarded by `[ -n "${SANDESH_ADMIN:-}" ]`.
        reindex must run unconditionally, not only when SANDESH_ADMIN is set.

        RED: no reindex line yet.
        """
        admin_start = self._line_index("SANDESH_ADMIN")
        reindex_idx = self._line_index("reindex")

        self.assertGreater(
            reindex_idx, -1,
            "install.sh has no 'reindex' line. GREEN must add it.",
        )

        if admin_start == -1:
            return  # No admin block — placement is fine.

        # Find the 'fi' closing the admin if-block.
        admin_fi_idx = -1
        for i in range(admin_start + 1, len(self._lines)):
            stripped = self._lines[i].strip()
            if stripped == "fi":
                admin_fi_idx = i
                break
        ...
        if admin_fi_idx == -1:
            return  # Cannot determine block end.

        # The reindex must NOT be inside the admin block.
        inside_admin = admin_start < reindex_idx < admin_fi_idx
        self.assertFalse(
            inside_admin,
            f"reindex (line {reindex_idx + 1}) is inside the $SANDESH_ADMIN block "
            f"(lines {admin_start + 1}–{admin_fi_idx + 1}). "
            "reindex must run unconditionally, not only when SANDESH_ADMIN is set.",
        )
