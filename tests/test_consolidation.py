...
# ---------------------------------------------------------------------------
# T10: install.sh content assertions
# ---------------------------------------------------------------------------

_INSTALL_SH = os.path.join(_REPO_ROOT, "install.sh")


class InstallShConsolidateTest(unittest.TestCase):
    """install.sh must contain a `consolidate` invocation that:
    1. Exists in the file.
    2. Is placed AFTER the migrate block (the 'migrate --all' line).
    3. Is NOT inside the yoyo-probe conditional (the
       'if "$VENV/bin/python" -c "import yoyo' block).

    RED: install.sh currently has no consolidate invocation at all.
    """
    ...
    def test_consolidate_invocation_exists(self):
        """install.sh must contain a 'consolidate' invocation.

        RED: no consolidate call exists yet in install.sh.
        """
        idx = self._line_index("consolidate")
        self.assertGreater(
            idx, -1,
            "install.sh contains no 'consolidate' invocation. "
            "GREEN must add: `\"$VENV/bin/sandesh\" consolidate` after the migrate block.",
        )

    def test_consolidate_placed_after_migrate_block(self):
        """The consolidate invocation must appear AFTER the migrate --all line.

        RED: no consolidate line exists → _line_index returns -1.
        """
        migrate_idx = self._line_index("migrate --all")
        consolidate_idx = self._line_index("consolidate")

        self.assertGreater(
            migrate_idx, -1,
            "install.sh has no 'migrate --all' line — cannot verify ordering.",
        )
        self.assertGreater(
            consolidate_idx, -1,
            "install.sh has no 'consolidate' line. GREEN must add it after migrate.",
        )
        self.assertGreater(
            consolidate_idx, migrate_idx,
            f"consolidate (line {consolidate_idx + 1}) must appear AFTER "
            f"migrate --all (line {migrate_idx + 1}).",
        )

    def test_consolidate_not_inside_yoyo_probe_conditional(self):
        """The consolidate invocation must NOT be inside the yoyo-probe
        if-block (which is guarded by `import yoyo`).

        The yoyo probe block starts at:
          if "$VENV/bin/python" -c "import yoyo, jsonschema" ...
        and ends at `fi`. Consolidate must appear OUTSIDE that block
        (it is stdlib — does not need the [migrate] extra).

        RED: no consolidate line exists yet.
        """
        # Find the yoyo probe start and end
        yoyo_start = self._line_index("import yoyo")
        consolidate_idx = self._line_index("consolidate")

        self.assertGreater(
            consolidate_idx, -1,
            "install.sh has no 'consolidate' line. GREEN must add it outside the yoyo block.",
        )

        if yoyo_start == -1:
            # No yoyo probe → consolidate placement is fine wherever it is
            return

        # Find the 'fi' that closes the yoyo if-block (first 'fi' after yoyo_start
        # at the same indentation level).
        yoyo_fi_idx = -1
        # The yoyo block starts with 'if "..."' and ends with a bare 'fi'
        # Scan for the closing fi
        ...
        if yoyo_fi_idx == -1:
            # Cannot determine block end — skip placement check
            return

        inside_yoyo = yoyo_start < consolidate_idx < yoyo_fi_idx
        self.assertFalse(
            inside_yoyo,
            f"consolidate (line {consolidate_idx + 1}) is inside the yoyo-probe "
            f"conditional (lines {yoyo_start + 1}–{yoyo_fi_idx + 1}). "
            f"Consolidate is stdlib-only and must NOT require the [migrate] extra.",
        )
